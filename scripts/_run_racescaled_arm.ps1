...
$LOG = "C:\dev\citiusverse\citiusdata\racescaled_arm_log.txt"
"=== START $(Get-Date) ===" | Out-File -Append -Encoding utf8 $LOG

$env:CITIUS_BT_CALIBRATION   = "calibration_racescaled.rds"
$env:CITIUS_BT_ADJUST_RACE   = "1"
$env:CITIUS_BT_STORE         = "athletics_corpus_store"
$env:CITIUS_BT_CACHE         = "bt_cache_racescaled"
$env:CITIUS_BT_OUT           = "backtest_racescaled.rds"
...
for ($i = 1; $i -le 6; $i++) {
  ...
  if (Test-Path "citiusdata\data\backtest_racescaled.rds") {
    $f = Get-Item "citiusdata\data\backtest_racescaled.rds"
...
$env:CITIUS_GOAL_ARM = "backtest_racescaled.rds"
...
Copy-Item "citiusdata\data\goal_by_event.csv" "citiusdata\data\goal_by_event_racescaled.csv" -Force
