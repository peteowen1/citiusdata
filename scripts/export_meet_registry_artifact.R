...
# `meet_strength` so the career-best and EW bases can be compared on the page: they
# disagree by ~5 on the median meet and by 20+ on the fast road courses, which
# is the whole point of the 2026-09-03 change.
.cols <- c("competition_id", "comp_name", "meet_type", "meet_strength", "strength_pb",
...
# Never-scored, never-classified meets (meet_strength NA AND meet_type unclassified)
...
trimmed <- keep[!(is.na(meet_strength) & meet_type == "unclassified")]
