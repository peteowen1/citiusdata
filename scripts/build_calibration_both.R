# Calibration for the `both` arm: meet_tier-fitted tier offsets AND per-event
# round/tier offsets, so the two adopted-candidate corrections can be tested
# together rather than assumed additive.
#
...
# BOTH corrections at once: tier offsets fitted on meet_tier AND round/tier
# offsets fitted per EVENT. mtier gave -0.47% and cevent -0.79%, both on the
# level rather than the ordering, so they may well be correcting the same bias
# twice rather than adding. That is the question this arm exists to answer.
...
saveRDS(cal, file.path(OUT, "calibration_corpus_both.rds"))
say("wrote calibration_corpus_both.rds")
