...
$LOG = "C:\dev\citiusverse\citiusdata\iter2_arm_log.txt"
"=== START $(Get-Date) ===" | Out-File -Append -Encoding utf8 $LOG

$env:CITIUS_BT_CALIBRATION   = "calibration_iter2.rds"
$env:CITIUS_BT_ADJUST_RACE   = "1"
$env:CITIUS_BT_STORE         = "athletics_corpus_store"
$env:CITIUS_BT_CACHE         = "bt_cache_iter2"
$env:CITIUS_BT_OUT           = "backtest_iter2.rds"
...
for ($i = 1; $i -le 6; $i++) {
  ...
  if (Test-Path "citiusdata\data\backtest_iter2.rds") {
    $f = Get-Item "citiusdata\data\backtest_iter2.rds"
...
$env:CITIUS_GOAL_ARM = "backtest_iter2.rds"
...
Copy-Item "citiusdata\data\goal_by_event.csv" "citiusdata\data\goal_by_event_iter2.csv" -Force
