...
(function (root) {
  ...
  // stadium: venue_stadium string or null; "city|stadium" is looked up first.
  function adjustConditions(params, wind, alt_m, indoor, venue, stadium) {
    ...
    let venue_off = 0
    if (venue != null && params.venues && Number.isFinite(params.venues[venue])) {
      venue_off = params.venues[venue]
      const k = venue + "|" + stadium
      if (stadium != null && params.stadiums && Number.isFinite(params.stadiums[k])) venue_off = params.stadiums[k]
    }
  ...
  function adjustRace(params, rows, var_resid = params.var_resid, loo = true) {
    const out = rows.map(r => {
      const a = adjustConditions(params, r.wind, r.alt_m, r.indoor, r.venue, r.stadium)
